...
function DropZone({onLimitReached}) {
      ...
      {files.length > 0 && (
        <ul className="mt-4">
          {files.map((file, index) => (
            <li key={index} className="flex items-center font-dm">
              <span className="font-dm">{file.name}</span> - {(file.size / 1048576).toFixed(2)} MB
              <button onClick={() => setFiles(files => files.filter((e, i) => i !== index))}
