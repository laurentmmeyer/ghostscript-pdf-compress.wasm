...
function _GSPS2PDF(
  ...
  xhr.onerror = function () {
    if (errorCallback) errorCallback(new Error("Failed to load PDF file"));
  };
  xhr.onload = async function () {
    try {
      ...
      Module = {
        print: function (text) {
          if (statusUpdateCallback) statusUpdateCallback(text);
        },
        printErr: function (text) {
          if (statusUpdateCallback) statusUpdateCallback("Error: " + text);
          console.error(text);
        },
      ...
      console.error("Ghostscript processing error:", error);
...
self.addEventListener('message', function ({data: e}) {
  ...
  _GSPS2PDF(
    e.data,
    ({pdfDataURL}) => self.postMessage(pdfDataURL),
    null,
    null,
    (error) => self.postMessage({error: error.message || "Compression failed"})
  );
});
